...
#include "holmes/net/tcp/policy_linux_2_4.h"

namespace holmes::net::tcp {

bool policy_linux_2_4::choose(uint64_t seq, uint64_t curseq,
	...
	// but the new segment is longer than the old segment, then accept the
	// new segment.
	if (new_seg.seq(seq) == old_seg->seq(seq)) {
